...
export const HEDERA_FAUCET_URL = "https://portal.hedera.com/";
...
const evmAccountCache = new Map<string, Promise<string | null>>();
...
export async function hederaAccountFromEvm(evmAddress: string) {
  ...
  const cached = evmAccountCache.get(address);
  ...
  const request = (async () => {
    try {
      ...
      evmAccountCache.delete(address);
      return null;
    }
  })();

  evmAccountCache.set(address, request);
  return request;
}
